...
app.controller('ProjectIndexController', [
  '$scope', '$location', 'angularFire', 'userService', 'projectService', '$http', 'firebase_settings'
  , function($scope, $location, angularFire, userService, projectService, $http, firebase_settings){
  ...
  $scope.loadPivotalProjects = function(){
    ...
    $http({method: 'GET', url:'https://www.pivotaltracker.com/services/v5/projects'}).then(function(res) { 
      ...
      angular.forEach(res.data, function(value, key){
        $scope.myUser.projects[value.id] = {name: value.name, id: value.id, point_scale: value.point_scale};
        $scope.projects[value.id] = {name: value.name, id: value.id, point_scale: value.point_scale};
      });
    });
  };
}]);
